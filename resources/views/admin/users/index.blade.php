...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .alert-error { background: #fee2e2; color: #991b1b; border: 1px solid #fca5a5; padding: 12px 16px; border-radius: 8px; font-size: 14px; margin-bottom: 20px; }
        ...
        .btn-submit { background: var(--navy); color: white; border: none; padding: 10px 24px; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; width: 100%; margin-top: 4px; }
        ...
        .role-badge { display: inline-block; font-size: 11px; padding: 2px 8px; border-radius: 10px; font-weight: 600; }
        .btn-delete { background: var(--accent); color: white; border: none; padding: 5px 12px; border-radius: 6px; cursor: pointer; font-size: 12px; }
        .btn-delete:hover { background: var(--accent-hover); }
        .self-label { font-size: 12px; color: #9ca3af; font-style: italic; }
    </style>
</head>
<body>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $user->email }}</td>
                            <td>
                                <span class="role-badge {{ $user->isAdmin() ? 'badge-admin' : 'badge-user' }}">
                            ...
                            <td>
                                @if($user->id !== Auth::id())
                                <form method="POST" action="/admin/users/{{ $user->id }}" onsubmit="return confirm('Hapus user {{ addslashes($user->name) }}?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn-delete">Hapus</button>
                                </form>
                                @else
                                <span class="self-label">Anda</span>
                                @endif
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="6" style="text-align:center;color:#9ca3af;padding:24px;">Belum ada user.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
